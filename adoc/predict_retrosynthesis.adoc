...
This guide is a short tutorial to quickly learn how to perform a predictive retrosynthesis with  RXN for Chemistry.
////
...
.*Step 4:* Select "Target molecule" to specify a specific molecule/compound.
[%collapsible]
====
NOTE: If you have a file containing SMILES strings you may use the "From file" option to upload that file for use.
...
[%collapsible]
====
On this page you will paste a SMILES string representing the "target" molecule to use for the retrosynthesis analysis. Use the one shown below. 
...
[NOTE]
======
*Automatic mode*: All of the most likely reactions and their reagents are predicted and the presented to you in a set of tables, allowing you to navigate and view each of the predicted retrosynthetic reactions. 

*Interactive mode*: The interactive mode allows you to select step-by-step the reagents and reaction steps that are most likely to result in the target molecule.   
======
...
NOTE: AIZ and RXN are both machine learning models that have been trained on slightly different data sets.  Both of these models are powerful tools for retrosynthesis analysis.  

After selecting the AI model version, you may move the slide control labeled "Speed/Quality Tuning" to set a balance between the rapid completion of results versus detailed and exhaustive results.

NOTE: Specific guidance and documentation for each of these machine learning models will be made available in Q4 2023.  
...
.*Step 22:* Click on the icon shown below to view Alternative Reactions (if any) then close Alternative Reactions sidebar 
[%collapsible]
====
...
.*Step 24:* Click on the TMAP icon as shown below 
[%collapsible]
====
...
* Executing a retrosynthesis analysis
* Viewing the results in a Project
* Viewing the details of the predicted retrosynthesis reactions
* Viewing the TreeMAP of retrosynthesis reactions
==== 
